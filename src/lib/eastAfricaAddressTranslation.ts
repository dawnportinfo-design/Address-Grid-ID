...
function normalizeEastAfricaEnglish(text: string, countryCode: string, fieldKey: string) {
...
export async function translateEastAfricaAddressField(options: {
  ...
  return translateAddressFieldByRoute({
    ...
    sourceLanguage,
    targetLanguage,
    normalizeEnglish: value => normalizeEastAfricaEnglish(value, profile.countryCode, options.fieldKey),
