...
const asiaDir = join(process.cwd(), 'src', 'data', 'address_formats', 'asia');
const requiredOpenSourceIds = ['osm-nominatim', 'openaddresses', 'geonames-postal'];

function collectFiles(extension: '.json' | '.yaml', dir = asiaDir): string[] {
  const files: string[] = [];
...
test('Asia address formats have renderable language tabs and open postal/geographic evidence', () => {
  const jsonFiles = collectFiles('.json');
  assert.equal(jsonFiles.length, 55);

  for (const filePath of jsonFiles) {
    const format = loadAddressFormat(filePath);
    const relativePath = relative(asiaDir, filePath).replace(/\\/g, '/');
...
test('Asia postal-code-required rules are reflected in every address language tab', () => {
