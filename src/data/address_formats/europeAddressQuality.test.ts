...
const europeDir = join(process.cwd(), 'src', 'data', 'address_formats', 'europe');
const requiredOpenSourceIds = ['osm-nominatim', 'openaddresses', 'geonames-postal'];

function collectFiles(extension: '.json' | '.yaml', dir = europeDir): string[] {
  const files: string[] = [];
...
test('Europe address formats have renderable language tabs and open postal/geographic evidence', () => {
  const jsonFiles = collectFiles('.json');
  assert.equal(jsonFiles.length, 70);

  for (const filePath of jsonFiles) {
    const format = loadAddressFormat(filePath);
    const relativePath = relative(europeDir, filePath).replace(/\\/g, '/');
    const expectedCountryCode = basename(filePath, '.json');

    assert.ok(
      expectedCountryCode === format.countryCode || expectedCountryCode.startsWith(`${format.countryCode}_`),
      `${relativePath} countryCode should match filename or its territory suffix`,
    );
...
test('Europe postal-code-required rules are reflected in every address language tab', () => {
