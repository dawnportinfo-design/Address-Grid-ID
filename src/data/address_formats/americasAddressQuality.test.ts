...
const americasDir = join(process.cwd(), 'src', 'data', 'address_formats', 'americas');
const requiredOpenSourceIds = ['osm-nominatim', 'openaddresses', 'geonames-postal'];

function collectFiles(extension: '.json' | '.yaml', dir = americasDir): string[] {
  const files: string[] = [];
...
function templateFields(addressFormat?: string): string[] {
  const fields: string[] = [];
  const seen = new Set<string>();

  for (const match of addressFormat?.matchAll(/\{\{([A-Za-z0-9_]+)\}\}/g) ?? []) {
    const field = match[1];
    if (field !== 'country' && !seen.has(field)) {
      fields.push(field);
      seen.add(field);
    }
  }
...
test('Americas address formats have renderable language tabs and open postal/geographic evidence', () => {
  const jsonFiles = collectFiles('.json');
  assert.equal(jsonFiles.length, 65);

  for (const filePath of jsonFiles) {
    const format = loadAddressFormat(filePath);
    const relativePath = relative(americasDir, filePath).replace(/\\/g, '/');
...
test('Americas language tabs expose fields for every rendered address placeholder', () => {
  ...
  for (const filePath of files) {
    ...
    for (const [languageKey, languageFormat] of listLanguageFormats(format)) {
      const placeholders = templateFields(languageFormat.addressFormat);
      if (placeholders.length === 0) continue;

      const fieldKeys = new Set((languageFormat.fields ?? []).map(field => field.key));
      for (const placeholder of placeholders) {
        assert.ok(
          fieldKeys.has(placeholder),
          `${relativePath} ${languageKey} should expose an input field for {{${placeholder}}}`,
        );
      }
    }
  }
...
test('Americas postal-code-required rules are reflected in every address language tab', () => {
  const files = [...collectFiles('.json'), ...collectFiles('.yaml')];
