...
const oceaniaDir = join(process.cwd(), 'src', 'data', 'address_formats', 'oceania');
const requiredOpenSourceIds = ['osm-nominatim', 'openaddresses', 'geonames-postal'];

function collectFiles(extension: '.json' | '.yaml', dir = oceaniaDir): string[] {
  const files: string[] = [];
...
test('Oceania address formats have renderable language tabs and open postal/geographic evidence', () => {
  const jsonFiles = collectFiles('.json');
  assert.equal(jsonFiles.length, 24);

  for (const filePath of jsonFiles) {
    const format = loadAddressFormat(filePath);
    const relativePath = relative(oceaniaDir, filePath).replace(/\\/g, '/');
...
test('Oceania language tabs expose fields for every rendered address placeholder', () => {
...
test('Oceania postal-code-required rules are reflected in every address language tab', () => {
  const files = [...collectFiles('.json'), ...collectFiles('.yaml')];
